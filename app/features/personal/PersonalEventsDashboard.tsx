...
const PersonalEventsDashboard = () => {
    ...
    const [stats, setStats] = useState<EventStats>({ disponibles: 0, enProgreso: 0, completados: 0 });
    ...
    const loadEvents = async () => {
        try {
            ...
            // Filter only 'evento_personal' type activities
            const personalEvents = (activitiesData as Activity[]).filter(
                (activity: Activity) => activity.type === 'evento_personal'
            );
            console.log('Filtered personal events:', personalEvents.length);
        ...
        return (
            <SafeAreaView style={[styles.container, tailwind('flex-1 bg-gray-50')]}>
                <View style={tailwind('flex-1 justify-center items-center')}>
                    ...
                    <Text style={tailwind('mt-4 text-gray-500')}>Cargando eventos...</Text>
                </View>
            </SafeAreaView>
        );
    }
        ...
        <SafeAreaView style={[styles.container, tailwind('flex-1 bg-gray-50')]}>
            <ScrollView contentContainerStyle={tailwind('py-4 px-4')} showsVerticalScrollIndicator={false}>
                ...
                <View style={tailwind('flex-row items-center mb-4')}>
                    ...
                    <Text style={tailwind('text-2xl font-bold text-gray-800 ml-3')}>E-Personal</Text>
                </View>
                ...
                <View style={tailwind('flex-row gap-3 mb-4')}>
                    <View style={[styles.statCard, tailwind('flex-1 bg-purple-500 rounded-xl p-4 items-center')]}>
                        <Text style={tailwind('text-3xl font-bold text-white')}>{stats.disponibles}</Text>
                        <Text style={tailwind('text-sm text-purple-100 mt-1')}>Disponibles</Text>
                    </View>
                ...
                {events.length === 0 ? (
                    <View style={tailwind('bg-white rounded-xl p-8 items-center shadow-sm')}>
                        ...
                        <Text style={tailwind('mt-4 text-gray-500 text-lg')}>
                            {selectedDate ? 'No hay eventos para esta fecha' : 'No hay eventos disponibles'}
                        </Text>
                        <Text style={tailwind('mt-2 text-gray-400 text-sm')}>
                            {selectedDate ? 'Selecciona otra fecha o ver todos' : 'Vuelve más tarde para nuevos eventos'}
                        </Text>
                    </View>
                ) : (
                    events.map((event) => (
                        <View key={event._id} style={tailwind('bg-white rounded-xl p-4 mb-3 shadow-sm')}>
                            <View style={tailwind('flex-row items-start justify-between')}>
                                <View style={tailwind('flex-row items-center flex-1')}>
                                    <Text style={tailwind('text-2xl mr-3')}>{event.emotion?.icono || '📅'}</Text>
                                    <View style={tailwind('flex-1')}>
                                        <Text style={tailwind('text-lg font-semibold text-gray-800')}>{event.title}</Text>
                            ...
                            <View style={tailwind('mt-4')}>
                                <View style={tailwind('flex-row justify-between mb-1')}>
                                    ...
                                    <Text style={tailwind('text-xs text-gray-500')}>0/{event.questions.length}</Text>
                                </View>
                                <View style={tailwind('h-2 bg-gray-200 rounded-full')}>
                                    <View style={[tailwind('h-2 bg-purple-500 rounded-full'), { width: '0%' }]} />
                                </View>
                            </View>

                            <TouchableOpacity style={tailwind('mt-4 bg-purple-500 rounded-lg py-3 items-center')}>
                ...
                <View style={tailwind('bg-white rounded-xl p-4 mt-6 shadow-sm')}>
                    <View style={tailwind('flex-row items-center mb-4')}>
                        <MaterialIcons name="leaderboard" size={24} color="#8B5CF6" />
